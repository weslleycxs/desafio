...
exports.kugel = {
    ...
    compileViews(){
        ...
        if(!global.package.kugel.config.compileViews) return;

        let compileViewsPath = path.join(global.dir.app, global.package.kugel.config.compileViews);
        ...
        return new Promise((resolve, reject) => {
            ...
            folders.forEach(folder => {
                ...
                let compiledFolder = path.join(global.dir.app, global.package.kugel.config.compiledViewsDest, folder);
            ...
            files.forEach(file => {
                ...
                global.package.kugel.config.viewsOptions.require = require;

                let viewContent = pug.renderFile(file, global.package.kugel.config.viewsOptions);

                let viewFilename = file.replace(compileViewsPath, '');

                viewFilename = viewFilename.replace(path.extname(viewFilename), '.html');

                let compiledViewDest = path.join(global.dir.app, global.package.kugel.config.compiledViewsDest, viewFilename);
